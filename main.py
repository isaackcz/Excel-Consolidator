...
class ConsolidationWorker(QThread):
    ...
    def __init__(self, template_path, excel_folder, save_folder, settings=None):
        super().__init__()
    ...
    def run(self):
        try:
            ...
            if not files:
                self.finished.emit("error", "No valid Excel files found in the folder.")
                return
                        ...
                        if validate_structure and template_ws is not None:
                            try:
                                if (ws.max_row != template_ws.max_row) or (ws.max_column != template_ws.max_column):
                                    if stop_on_error:
                                        wb.close()
                                        self.finished.emit("error", f"Structure mismatch in {os.path.basename(file)}")
                                        return
                                    wb.close()
                        ...
                        for cell in cells_iter:
                            ...
                            if val is None:
                                ...
                                if validate_data_types and (cell.value not in (None, "")):
                                    if stop_on_error:
                                        wb.close()
                                        self.finished.emit("error", f"Non-numeric data at {cell.coordinate} in {os.path.basename(file)}")
                                        return
                            ...
                            if validate_value is not None and not validate_value(val, self.settings):
                                if self.settings.get('validation', {}).get('stop_on_error'):
                                    wb.close()
                                    self.finished.emit("error", f"Value {val} out of allowed range at {cell.coordinate} in {os.path.basename(file)}")
                                    return
                                continue
                ...
                try:
                    for coord in totals.keys():
                        ...
                        if first_row:
                            ...
                            output_ws[coord].hyperlink = link
                            # Optional tooltip
                            output_ws[coord].hyperlink.tooltip = "Click to view full contributions"
                except Exception:
                    pass
...
class ExcelProcessorApp(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Excel Consolidator")
        self.setGeometry(400, 200, 900, 650)
        
        # Update icon loading to use resource_path
        icon_path = self.resource_path("resources/app.ico")
    ...
    def apply_modern_style(self):
        check_svg = self.resource_path("resources/check.svg").replace("\\", "/")
        check_disabled_svg = self.resource_path("resources/check_disabled.svg").replace("\\", "/")
        css = """
            /* Base */
            QWidget {
    ...
    def run_processing(self):
        if not self.template_path or not self.excel_folder or not self.save_folder:
            QMessageBox.warning(self, "Missing Information", 
                               "Please complete all three steps before running the consolidation.")
            return
